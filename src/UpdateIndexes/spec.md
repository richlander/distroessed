...
Note: At present, this tool is a prototype. It is using the richlander/core repo instead of the dotnet/core one. Once the tool and (more importantly) the generated resources are considered "ready", then the URLs will switch to the dotnet/core repo (and this note will be deleted). A separate (personal) repo is being used because chat assistants seem unable to use branch URLs, particularly via `raw.githubusercontent.com` URLs.
